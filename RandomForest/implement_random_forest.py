...
def spiltDataSet(dataSet, n_folds):  # 将数据集随机分成N块，方便交叉验证，其中一块是测试集，其他四块是训练集
	...
	for i in range(n_folds):
		...
		while len(fold) < fold_size:
			...
			fold.append(dataset_copy.pop(index))  # pop是删掉后并返回该值,和remove不一样
...
def main():
	...
	n_features = 15
	n_trees = 10
	dataSet = loadCSV('sonar-all-data.csv')  # 并不是一个df,而是数组
	column_to_float(dataSet)  # dataSet

	folds = spiltDataSet(dataSet, n_folds)  # folds的尺寸为5*41*61
	scores = []
	for fold in folds:
		train_set = folds[:]
